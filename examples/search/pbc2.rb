...
while a = circles_dup.shift
  circles_dup.each {|b|
    ...
    if a.radius + b.radius <= d
      ...
      svg << l1.to_svg('stroke:red') if Circular.visible?(l1, circles, a, b)
      svg << l2.to_svg('stroke:red') if Circular.visible?(l2, circles, a, b)
    end
    if (a.radius - b.radius).abs <= d
      l3, l4 = Circular.external_bitangent_lines(a, b, d)
      svg << l3.to_svg('stroke:blue') if Circular.visible?(l3, circles, a, b)
      svg << l4.to_svg('stroke:blue') if Circular.visible?(l4, circles, a, b)
    end
  }
end
p Time.now.to_f - t
